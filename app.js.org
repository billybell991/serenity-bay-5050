...
mapBtn.addEventListener('click', () => {
    ...
    // Ensure Leaflet recalculates the map size after the container becomes visible
    if (window._leafletMap) {
        // Trigger invalidate size a few times to ensure CSS transitions are done
        setTimeout(() => {
            window._leafletMap.invalidateSize();
            if (!mapInitialized) {
                // First time showing map, fit bounds
                window._leafletMap.fitBounds([[0, 0], [1790, 2400]]);
                mapInitialized = true;
            }
        }, 50);
    }
